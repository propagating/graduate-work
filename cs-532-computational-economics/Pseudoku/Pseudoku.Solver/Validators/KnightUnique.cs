    ...
    public class KnightUnique : IValidator
    {
        ...
        public bool ValidatePotentialCellValues(PseudoCell cell, PseudoBoard board)
        {
            ...
            foreach (var move in KnightMoves.ToList())
            {
                ...
                var existingValues = board.BoardCells.Where(x => x.CellRow == moveVertical && x.CellColumn == moveHorizontal && x.CellBox != cell.CellBox  && x.SolvedCell && cell.PossibleValues.Contains(x.CurrentValue)).Select(x=> x.CurrentValue).ToList();
