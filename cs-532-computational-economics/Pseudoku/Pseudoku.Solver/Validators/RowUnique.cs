...
namespace Pseudoku.Solver.Validators
{
    public class RowUnique : IValidator
    {
        ...
        {
            var existingValues = board.BoardCells.Where(x => x.CellRow == cell.CellRow && x.SolvedCell && cell.PossibleValues.Contains(x.CurrentValue)).Select(x=> x.CurrentValue).ToList();
