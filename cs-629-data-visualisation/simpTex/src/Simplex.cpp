...
float generate_simplex_gradient(int hash, float x) 
{
	auto h = hash & 15;
...
float generate_simplex_gradient(int hash, float x, float y) 
{
	auto h = hash & 7;      // Convert low 3 bits of hash code
	auto u = h < 4 ? x : y;  // into 8 simple gradient directions,
	auto v = h < 4 ? y : x;  // and compute the dot product with (x,y).
    return ((h & 1) ? -u : u) + ((h & 2) ? -2.0f * v : 2.0f * v);
}

float generate_simplex_gradient(int hash, float x, float y, float z) 
{
	auto h = hash & 15;     // Convert low 4 bits of hash code into 12 simple
	auto u = h < 8 ? x : y; // gradient directions, and compute dot product.
	auto v = h < 4 ? y : h == 12 || h == 14 ? x : z; // Fix repeats at h = 12 to 15
    return ((h & 1) ? -u : u) + ((h & 2) ? -v : v);
}

float generate_simplex_gradient(int hash, float x, float y, float z, float t) 
{
...
// 1D simplex noise
float generate_simplex_one_dim(float x)
{
	auto i0 = FASTFLOOR(x);
	auto i1 = i0 + 1;
	auto x0 = x - i0;
    ...
    //  if(t0 < 0.0f) t0 = 0.0f;
    t0 *= t0;
    n0 = t0 * t0 * generate_simplex_gradient(perm[i0 & 0xff], x0);

	auto t1 = 1.0f - x1 * x1;
    //  if(t1 < 0.0f) t1 = 0.0f;
    t1 *= t1;
    n1 = t1 * t1 * generate_simplex_gradient(perm[i1 & 0xff], x1);
...
#define G2 0.211324865f // G2 = (3.0-Math.sqrt(3.0))/6.0
    ...
    auto s = (x + y) * F2; // Hairy factor for 2D
    auto xs = x + s;
    auto ys = y + s;
    auto i = FASTFLOOR(xs);
    auto j = FASTFLOOR(ys);
    ...
    if (t0 < 0.0f) n0 = 0.0f;
        ...
        t0 *= t0;
        n0 = t0 * t0 * generate_simplex_gradient(perm[ii + perm[jj]], x0, y0);
    }
    ...
    if (t1 < 0.0f) n1 = 0.0f;
        ...
        t1 *= t1;
        n1 = t1 * t1 * generate_simplex_gradient(perm[ii + i1 + perm[jj + j1]], x1, y1);
    }
    ...
    if (t2 < 0.0f) n2 = 0.0f;
        ...
        t2 *= t2;
        n2 = t2 * t2 * generate_simplex_gradient(perm[ii + 1 + perm[jj + 1]], x2, y2);
    }
...
// 3D simplex noise
float generate_simplex_three_dim(float x, float y, float z)
{
...
/* This code would benefit from a backport from the GLSL version! */
    ...
    if (t0 < 0.0f) n0 = 0.0f;
        ...
        t0 *= t0;
        n0 = t0 * t0 * generate_simplex_gradient(perm[ii + perm[jj + perm[kk]]], x0, y0, z0);
    }
    ...
    if (t1 < 0.0f) n1 = 0.0f;
        ...
        t1 *= t1;
        n1 = t1 * t1 * generate_simplex_gradient(perm[ii + i1 + perm[jj + j1 + perm[kk + k1]]], x1, y1, z1);
    }
    ...
    if (t2 < 0.0f) n2 = 0.0f;
        ...
        t2 *= t2;
        n2 = t2 * t2 * generate_simplex_gradient(perm[ii + i2 + perm[jj + j2 + perm[kk + k2]]], x2, y2, z2);
    }
    ...
    if (t3 < 0.0f) n3 = 0.0f;
        ...
        t3 *= t3;
        n3 = t3 * t3 * generate_simplex_gradient(perm[ii + 1 + perm[jj + 1 + perm[kk + 1]]], x3, y3, z3);
    }
